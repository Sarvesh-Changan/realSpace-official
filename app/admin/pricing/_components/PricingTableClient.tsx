    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-neutral-500">Manage base prices and multiplier rules for the public calculator.</p>
        </div>
        <button onClick={() => onAdd()} className="inline-flex items-center px-4 py-2 bg-brand-red text-white text-sm font-medium rounded-md hover:bg-brand-red/90 transition-colors">
        ...
        {Object.entries(GROUP_LABELS).map(([groupKey, groupLabel]) => {
          ...
          return (
            <div key={groupKey} className="bg-white border border-neutral-200 rounded-lg shadow-sm overflow-hidden">
              <div 
                className="flex items-center justify-between px-4 py-3 bg-neutral-50 border-b border-neutral-200 cursor-pointer select-none hover:bg-neutral-100 transition-colors"
                onClick={() => toggleGroup(groupKey)}
              >
                <div className="flex items-center gap-2">
                  {isExpanded ? <ChevronDown className="w-4 h-4 text-neutral-500" /> : <ChevronRight className="w-4 h-4 text-neutral-500" />}
                  <h3 className="font-semibold text-neutral-900">{groupLabel}</h3>
                ...
                <button 
                  ...
                  className="text-xs font-medium text-brand-red hover:text-brand-red/80 flex items-center gap-1"
                >
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-white border-b border-neutral-100 text-xs text-neutral-400 font-medium">
                        ...
                        <th className="px-4 py-2 text-center w-24">Order</th>
                        <th className="px-4 py-2 text-center w-24">Active</th>
                    ...
                    <tbody className="divide-y divide-neutral-100 text-sm">
                      {groupItems.map(item => (
                        <tr key={item.id} className="hover:bg-neutral-50/50">
                          <td className="px-4 py-2.5 font-medium text-neutral-800">{item.label}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-neutral-600">{formatCurrency(item.basePrice)}</td>
                          <td className="px-4 py-2.5 text-right font-mono text-neutral-500">{item.perUnitPrice ? formatCurrency(item.perUnitPrice) : "—"}</td>
                          <td className="px-4 py-2.5 text-center text-neutral-400">{item.sortOrder}</td>
                          <td className="px-4 py-2.5 text-center">
                            <label className="relative inline-flex items-center cursor-pointer">
                              ...
                              <div className="w-7 h-4 bg-neutral-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-green-500"></div>
                            </label>
                          </td>
                          <td className="px-4 py-2.5 text-right space-x-2">
                            <button onClick={() => onEdit(item.id)} className="text-neutral-400 hover:text-brand-red transition-colors" title="Edit">
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button onClick={() => onDelete(item.id)} className="text-neutral-400 hover:text-red-600 transition-colors" title="Delete">

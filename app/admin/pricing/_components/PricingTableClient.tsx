...
export function PricingConfigTable({ options, onAdd, onEdit, onDelete, onToggleActive }: PricingConfigTableProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 sm:p-5 text-amber-900 text-sm flex items-start gap-3 shadow-xs">
        <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
        <div>
          <div className="font-bold text-amber-950 text-base mb-1">
            Archived Pricing Rules (Legacy — Inactive)
          </div>
          <p className="leading-relaxed text-amber-800 text-xs sm:text-sm">
            These legacy pricing rules are no longer used by the live quote calculator. Component pricing is now managed under the <strong>Component Tier Matrix</strong> tab.
          </p>
        </div>
      </div>

      <div className="opacity-70 grayscale-[35%] hover:grayscale-0 transition-all space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-neutral-900 flex items-center gap-2">
              Legacy Quote Rules <span className="text-xs bg-neutral-200 text-neutral-600 px-2 py-0.5 rounded uppercase tracking-wider font-semibold">Archived</span>
            </h2>
            <p className="text-xs sm:text-sm text-neutral-500">Historical base prices and multiplier rules (Read-only reference).</p>
          </div>
          <button onClick={() => onAdd()} className="inline-flex items-center justify-center px-4 py-2.5 min-h-[44px] bg-neutral-600 text-white text-xs sm:text-sm font-medium rounded-md hover:bg-neutral-700 transition-colors cursor-pointer w-full sm:w-auto">
              ...
              <div key={groupKey} className="bg-white border border-neutral-200 rounded-lg shadow-sm overflow-hidden">
                ...
                >
                  ...
                  <button
                    ...
                    className="text-xs font-medium text-neutral-600 hover:text-neutral-900 flex items-center gap-1 p-1 min-h-[36px] cursor-pointer"
                  >

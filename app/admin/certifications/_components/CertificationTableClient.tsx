...
import { Plus, Edit, Trash2, Award, ExternalLink, CheckCircle, XCircle } from "lucide-react";
            ...
            <table className="w-full text-left border-collapse min-w-[640px]">
              <thead>
                <tr className="bg-neutral-50 border-b border-neutral-200 text-xs font-semibold text-neutral-500 uppercase tracking-wider">
                  <th className="py-3.5 px-4 w-1/3">Certification Details</th>
                  <th className="py-3.5 px-4">Type & Badge</th>
                  <th className="py-3.5 px-4">Public Link & Button</th>
              ...
              <tbody className="divide-y divide-neutral-200 text-sm">
                {certifications.map((cert) => {
                  const logoSrc = cert.imageUrl && cert.imageUrl.trim() !== "" ? cert.imageUrl : "/images/certifications/cadpro.png";

                  return (
                    <tr key={cert.id} className="hover:bg-neutral-50/50 transition-colors">
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <div className="relative w-10 h-10 rounded border border-neutral-200 bg-neutral-50 overflow-hidden shrink-0 p-1">
                            <Image
                              src={logoSrc}
                              alt={cert.title}
                              fill
                              ...
                              unoptimized={!logoSrc.includes("res.cloudinary.com")}
                            />
                          </div>
                          <div>
                        ...
                        <div className="flex flex-col gap-1.5 items-start">
                          {cert.certificateUrl && cert.certificateUrl.trim() !== "" ? (
                            <a
                              ...
                              className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-200 hover:underline"
                            >
                              <ExternalLink className="w-3 h-3" /> Drive Link Attached
                            </a>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-neutral-50 text-neutral-400 border border-neutral-200">
                              No Drive Link
                            </span>
                          )}

                          {cert.showCertificateButton ? (
                            <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-200 font-medium">
                              <CheckCircle className="w-3 h-3" /> Button On
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded bg-neutral-50 text-neutral-400 border border-neutral-200">
                              <XCircle className="w-3 h-3" /> Button Off
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <button
                          ...
                          className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-brand-red focus:ring-offset-2 transition-colors ${
                            cert.isPublished ? "bg-brand-red" : "bg-neutral-300"
                          }`}
                          role="switch"
                        ...
                        >
                          ...
                          <span
                            ...
                            className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                              cert.isPublished ? "translate-x-2" : "-translate-x-2"
                            }`}
                          />
                        </button>
                      </td>

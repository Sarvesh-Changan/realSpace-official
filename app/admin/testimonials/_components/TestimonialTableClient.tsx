                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-neutral-50 border-b border-neutral-200 text-xs uppercase tracking-wider text-neutral-500 font-semibold">
                            <th className="px-4 py-3 w-1/4">Client</th>
                            <th className="px-4 py-3 w-1/3">Quote</th>
                            ...
                            <tr key={testimonial.id} className="hover:bg-neutral-50/50 transition-colors">
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-0.5">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <Star key={star} className={`w-3 h-3 ${star <= testimonial.rating ? "fill-brand-yellow text-brand-yellow" : "text-neutral-200"}`} />
                                        ))}
                                    </div>
                                </td>
                                ...
                                <td className="px-4 py-3 text-right space-x-2">
                                    <button onClick={() => onEdit(testimonial.id)} className="text-neutral-500 hover:text-brand-red transition-colors p-1" title="Edit">
                                        <Edit2 className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => onDelete(testimonial.id)} className="text-neutral-500 hover:text-red-600 transition-colors p-1" title="Delete">
